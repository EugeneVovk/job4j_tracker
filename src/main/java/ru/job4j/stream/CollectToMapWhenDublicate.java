...
/**
 * 2.8. Сборка элементов.
 * Сборка в Map, когда есть дубликаты
 * Метод, который был показан ранее нельзя использовать,
 * если стрим содержит дубликаты.
 * Для того чтобы собрать данные в мапу в этом случае,
 * мы можем использовать методы collect() и Collectors.toMap(),
 * но перегруженный.
 ...
 * В качестве третьего аргументы мы указываем, что делать в случае дубликата ключа,
 * оставим мы старое значение или новое
 * В примере, ключом и значением будет сам элемент,
 * а в случае дубликата мы указываем, что останется предыдущее значение
 * <p>
